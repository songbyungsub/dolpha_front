    ...
    <MKBox>
      ...
      {autotradingList.length === 0 ? (
            ...
            <AccordionDetails sx={{ backgroundColor: '#ffffff', position: 'relative' }}>
              ...
              <MKBox sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <MKBox>
                  ...
                  {Math.max(pyramidingCount, pyramidingEntries.length) > 0 && pyramidingEntries.map((entry, index) => (
        ...
        <MKBox>
          ...
          {selectedStock && !autotradingList.find(config => config.stock_code === selectedStock.code) && (
            <Accordion
              key={`new-${selectedStock.code}`}
              expanded={expandedAccordion === selectedStock.code}
              onChange={handleAccordionChange(selectedStock.code)}
              ...
              <AccordionDetails sx={{ backgroundColor: '#ffffff', position: 'relative' }}>
                ...
                <MKBox sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  <MKBox>
                    ...
                    {/* 피라미딩 진입시점과 포지션 */}
                    {pyramidingCount > 0 && pyramidingEntries.map((entry, index) => (
                      <Grid container spacing={1} key={index} sx={{ mb: 1 }}>
                        <Grid item xs={6}>
                          <TextField
                            label={`${index + 2}차 진입시점 (%)`}
